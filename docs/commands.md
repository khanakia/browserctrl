...
Serves the agent skill(s) this binary ships, always at the version that matches the binary. Provided by [voltkit/skillcmd](https://github.com/khanakia/voltkit/tree/main/skillcmd); the wiring is `cmd/browserctrl/skills_gen.go`, generated by `volt gen skills`.
...
Where the content comes from, first hit wins: the `BROWSERCTRL_SKILLS_DIR` environment variable; for a dev build (version `dev`, which includes any `go build` from a checkout) the nearest `skills/` directory walking up from the working directory; otherwise the per-version cache under `os.UserCacheDir()/browserctrl/skills/<version>/`, filled once from the `browserctrl/<version>` release's `skills_<version>.tar.gz` and checksum-verified. Every command says which source served.
